...
- **Multi-task learning model.** Consolidated several fraud tasks into one network — **shared embeddings + task-specific heads** → better generalization and **−60% training cost**. *Why MTL:* related tasks share signal, one model to maintain. *Risk you watched:* negative transfer, per-task metrics. 🔧 [LibMTL](https://github.com/median-research-group/LibMTL) (MTL library used)
